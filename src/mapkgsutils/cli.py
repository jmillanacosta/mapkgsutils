"""Command line interface for :mod:`mapkgsutils`.
...
- When you run ``python3 -m mapkgsutils`` python will execute``__main__.py`` as a
  script. That means there won't be any ``mapkgsutils.__main__`` in ``sys.modules``.
- When you import __main__ it will get executed again (as a module) because there's no
  ``mapkgsutils.__main__`` in ``sys.modules``.
